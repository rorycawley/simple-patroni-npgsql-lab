...
readonly NODE_FQDNS=(pg1.lab5.example pg2.lab5.example pg3.lab5.example)
readonly APP_VM=lab5-app1
readonly APP_FQDN=app1.lab5.example
...
require_vms() {
  command -v limactl >/dev/null 2>&1 || {
    echo "limactl is required to build the Lab 5 inventory" >&2
    exit 1
  }
  [[ -f "$ENV_FILE" ]] || {
...
inventory() {
  ...
  for index in "${!VM_NAMES[@]}"; do
    ...
    [[ "$index" -gt 0 ]] && printf ','
    printf '"%s":{"ansible_host":"lima-%s","ansible_ssh_common_args":"-F %s","lab5_node_ip":"%s","lab5_node_name":"%s","lab5_node_fqdn":"%s"}' \
  ...
  printf ',"%s":{"ansible_host":"lima-%s","ansible_ssh_common_args":"-F %s","lab5_node_ip":"%s","lab5_node_name":"app1","lab5_node_fqdn":"%s"}' \
