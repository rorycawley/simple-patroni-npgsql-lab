...
[Lab 5 guide](../README.md). This file covers how it is applied.
...
| `app-client.yml` | Provisions `lab5-app1` with the client, the CA, and the password file |
...
- installs `lab5-require-volume`, a guard the service units call from
  `ExecStartPre`.
...
backup design. Moving them off the database VM and encrypting them is Lab 5.

## The application host

`roles/app_client` installs the client on `lab5-app1` as a self-contained
...
`pg1.lab5.example`, `pg2.lab5.example`, `pg3.lab5.example` and `app1.lab5.example`
in macOS `/etc/hosts`.
