...
# Fault-injection tests for the Lab 5 cluster.
#
...
readonly FENCE_TABLE=lab5_fence
...
diagnose_election() {
  ...
  limactl shell --tty=false "$survivor" sudo bash -c \
    "etcdctl --cacert=/etc/lab5/pki/ca.crt --cert=/etc/lab5/pki/etcd.crt \
     --key=/etc/lab5/pki/etcd.key \
...
run_scenario() {
  ...
  client_stdout="$(mktemp "${TMPDIR:-/tmp}/lab5-failover-out.XXXXXX")"
  client_stderr="$(mktemp "${TMPDIR:-/tmp}/lab5-failover-err.XXXXXX")"
