...
while IFS= read -r label; do
  [[ -z "$label" ]] && continue
  found="$(on "$node" sudo "$BIN/lab5-s3" list "pgbackrest/backup/$STANZA/$label/" | grep -c . || true)"
  (( found > 0 )) || { fail "$label is listed by info but has no objects in the bucket"; missing=1; }
done <<< "$labels"
count="$(grep -c . <<< "$labels" || true)"
if (( count == 0 )); then
  # Never a pass. "Everything listed is present" is vacuously true of an empty
  # repository, and this check exists precisely to catch an empty one.
  fail "the repository lists no backups at all; there is nothing to be complete"
elif (( missing == 0 )); then
  pass "all $count backup(s) have objects in the bucket"
fi
...
# not just its rows -- and Lab 5's recovery depends on it, so it is asserted here
