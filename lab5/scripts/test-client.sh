...
run_client() {
  ...
  local -a env_pairs=(
    ...
    "LAB5_PGPASS=/etc/lab5/client/pgpass"
    "LAB5_CA=/etc/lab5/client/ca.crt"
  )
  while [[ "${1:-}" == *=* ]]; do env_pairs+=("$1"); shift; done
  limactl shell --tty=false lab5-app1 sudo env "${env_pairs[@]}" \
    /opt/lab5-client/Lab4.Client "$@"
}
...
test_uncertain_write() {
  ...
  app_name="lab5-uncertain-${probe_id:0:8}"
  ...
  client_stdout="$(mktemp "${TMPDIR:-/tmp}/lab5-uncertain-out.XXXXXX")"
  client_stderr="$(mktemp "${TMPDIR:-/tmp}/lab5-uncertain-err.XXXXXX")"

  run_client "LAB5_PROBE_ID=$probe_id" "LAB5_APP_NAME=$app_name" uncertain-write > "$client_stdout" 2> "$client_stderr" &
  client_pid=$!
