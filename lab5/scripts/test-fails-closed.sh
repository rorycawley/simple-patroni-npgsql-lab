...
restore_tampered() {
  if [[ -n "$tampered_key" ]] \
    ...
    if on "$leader" sudo bash -c "$BIN/lab5-s3 put /tmp/fc-original.bin '$tampered_key'" >/dev/null 2>&1; then
...
for _vm in "${VM_NAMES[@]}"; do
  on "$_vm" sudo systemctl stop lab5-backup-full.timer lab5-backup-incr.timer lab5-dump.timer \
    >/dev/null 2>&1
done
resume_timers() {
  local _vm
  for _vm in "${VM_NAMES[@]}"; do
    on "$_vm" sudo systemctl start lab5-backup-full.timer lab5-backup-incr.timer lab5-dump.timer \
      >/dev/null 2>&1
  done
}
...
on "$leader" sudo bash -c "$BIN/lab5-s3 cat '$tampered_key' > /tmp/fc-original.bin" >/dev/null 2>&1
...
on "$leader" sudo bash -c "$BIN/lab5-s3 put /tmp/fc-original.bin '$tampered_key'" >/dev/null 2>&1
