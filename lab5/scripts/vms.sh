...
readonly VM_HOSTNAMES=(pg1.lab5.example pg2.lab5.example pg3.lab5.example)
...
readonly APP_HOSTNAME=app1.lab5.example
...
readonly HOSTS_BEGIN="# BEGIN simple-patroni-npgsql-lab lab5"
readonly HOSTS_END="# END simple-patroni-npgsql-lab lab5"

usage() {
  cat <<'EOF'
Usage: ./scripts/vms.sh create|destroy|status|hostnames

create   Create (if needed) and start lab5-pg1, lab5-pg2, lab5-pg3, and lab5-app1.
destroy  Permanently delete those four VMs and their disks.
status   Display the Lima status of the Lab 5 VMs.
hostnames  Update the marked Lab 5 block in macOS /etc/hosts.
...
configure_hostnames() {
  ...
  hosts_without_block="$(mktemp "${TMPDIR:-/tmp}/lab5-hosts.XXXXXX")"
  updated_hosts="$(mktemp "${TMPDIR:-/tmp}/lab5-hosts-updated.XXXXXX")"
  ...
  [[ "${LAB5_INTERACTIVE_SUDO:-0}" == 1 ]] && sudo_options=()
...
remove_hostnames() {
  ...
  if ! sudo "${sudo_options[@]}" cp "$hosts_without_block" "$HOSTS_FILE"; then
    echo "Could not remove the Lab 5 host aliases without interactive sudo." >&2
    rm -f "$hosts_without_block"
    return
  fi
  rm -f "$hosts_without_block"
  echo "Removed Lab 5 hostnames from $HOSTS_FILE"
}
...
main() {
  ...
  case "${1:-}" in
    ...
    destroy) LAB5_INTERACTIVE_SUDO=1 destroy ;;
    status) status ;;
    hostnames) require_network; LAB5_INTERACTIVE_SUDO=1 configure_hostnames ;;
    *) usage; exit 2 ;;
  esac
}
