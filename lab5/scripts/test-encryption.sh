...
head_hex="$(on "$node" sudo bash -c \
  "$BIN/lab5-s3 cat pgbackrest/backup/$STANZA/backup.info 2>/dev/null | head -c 8 | od -An -v -tx1 | tr -d ' \n'")"
...
newest_dump="$(on "$node" sudo "$BIN/lab5-s3" list dumps/ | sort | tail -1)"
if [[ -z "$newest_dump" ]]; then
  fail "no dump found to check"
else
  head_hex="$(on "$node" sudo bash -c \
    "$BIN/lab5-s3 cat '$newest_dump' 2>/dev/null | head -c 8 | od -An -v -tx1 | tr -d ' \n'")"
  if [[ "$head_hex" == 53616c7465645f5f ]]; then
  ...
  wrong_head="$(on "$node" sudo bash -c \
    "$BIN/lab5-s3 cat '$newest_dump' > /tmp/lab5-enc-probe 2>/dev/null
     openssl enc -d -aes-256-cbc -pbkdf2 -in /tmp/lab5-enc-probe \
       -out /tmp/lab5-enc-probe.out -pass pass:'$repo_pass' 2>/dev/null
     head -c 5 /tmp/lab5-enc-probe.out 2>/dev/null")"
  [[ "$wrong_head" == "PGDMP" ]] \
  ...
  right_head="$(on "$node" sudo bash -c \
    "openssl enc -d -aes-256-cbc -pbkdf2 -in /tmp/lab5-enc-probe \
       -out /tmp/lab5-enc-probe.out -pass file:$PASSFILE 2>/dev/null
     head -c 5 /tmp/lab5-enc-probe.out 2>/dev/null")"
  [[ "$right_head" == "PGDMP" ]] \
  ...
  on "$node" sudo rm -f /tmp/lab5-enc-probe /tmp/lab5-enc-probe.out >/dev/null 2>&1
fi
