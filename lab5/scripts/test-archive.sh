...
readonly TIMERS=(lab5-backup-full.timer lab5-backup-incr.timer lab5-dump.timer)
...
for vm in "${VM_NAMES[@]}"; do
  on "$vm" sudo systemctl start "lab5-backup@incr.service" >/dev/null 2>&1
done
if on "$after_leader" journalctl -u "lab5-backup@incr.service" -n 20 --no-pager | grep -q "Leader confirmed"; then
