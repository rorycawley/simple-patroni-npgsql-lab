...
cleanup() {
  if [[ -n "$broken_vm" ]]; then
    ...
    limactl shell --tty=false "$broken_vm" sudo bash -c '
      ...
      cryptsetup status lab5-pgdata >/dev/null 2>&1 || \
        cryptsetup luksOpen --key-file /etc/lab5/luks/pgdata.key "$device" lab5-pgdata
      [ -f /etc/fstab.lab5bak ] && mv /etc/fstab.lab5bak /etc/fstab
...
for spec in "/var/lib/pgsql:lab5-pgdata" "/var/lib/etcd:lab5-etcd"; do
...
test_guard() {
  ...
  target=""
  for vm in "${VM_NAMES[@]}"; do
    if [[ "$vm" != "lab5-$leader" ]]; then target="$vm"; break; fi
  done
...
set -uo pipefail
fail=0
device=$(cryptsetup status lab5-pgdata | awk '/device:/ {print $2}')
...
check_case() {
  ...
  if [ "$state" = "active" ]; then
    if [ "$src" = "/dev/mapper/lab5-pgdata" ]; then
...
cryptsetup luksClose lab5-pgdata
...
cp /etc/fstab /etc/fstab.lab5bak
sed -i '/lab5-pgdata/d' /etc/fstab
systemctl daemon-reload
