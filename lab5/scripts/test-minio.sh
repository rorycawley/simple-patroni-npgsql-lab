...
readonly GUEST_CA=/etc/lab5/pki/ca.crt
...
echo
echo "=== This lab names no other lab ==="
# Forking a lab and half-renaming it is the failure mode this catches: Lab 3
# corrupted its own README that way, and a fork that still points at its parent
# shares a stanza -- which belongs to ONE database -- or a port, and then cannot
# be built while the parent runs. Executable files only; prose may cite whatever
# it likes.
# Comments are stripped first. A comment citing lab2/PLAN.md is provenance and
# harms nothing; a NAME that reaches another lab is what breaks a fork.
strays="$(find "$LAB_DIR/scripts" "$LAB_DIR/ansible" -type f 2>/dev/null; echo "$LAB_DIR/Makefile")"
strays="$(cat $strays 2>/dev/null | sed 's/#.*$//' \
  | grep -ohE 'lab[0-9]' | sort -u | grep -v '^lab5$' | tr '\n' ' ')"
if [[ -z "$strays" ]]; then
  pass "no executable file here names another lab"
else
  fail "executable files still name: $strays — a half-renamed fork shares a stanza or a port"
fi
...
if mc --quiet --no-color alias set lab5backup "$ENDPOINT" \
...
mc --quiet --no-color ls "lab5backup/$BUCKET" >/dev/null 2>&1 \
...
if mc --quiet --no-color mb "lab5backup/should-not-exist" >/dev/null 2>&1; then
  fail "the backup credential created a second bucket; the policy is not scoped"
  mc --quiet --no-color rb --force "lab5backup/should-not-exist" >/dev/null 2>&1 || true
else
