...
# The backup object store for Lab 5, on the control machine.
#
...
#   lab5-pgbackrest reads and writes ONE bucket, and nothing else
#
...
# premise of Lab 5 is that the cluster can be destroyed while the backups
...
# Not 9000 (the sibling lab ../percona-patroni-npgsql-lab holds it), not 9100
# (Lab 3) and not 9200 (Lab 4). Every lab that runs an object store needs its own
# port, or it cannot be built while another one is running -- the same standalone
# property that keeps Labs 1 and 2 independent copies.
readonly PORT="${LAB5_MINIO_PORT:-9300}"
readonly POLICY=lab5-backup-rw
...
do_start() {
  ...
  cp "$PKI_DIR/ca.crt" "$MC_DIR/certs/CAs/lab5-ca.crt"
  ...
  mc_ alias set lab5 "$(endpoint)" "$root_user" "$root_pass" >/dev/null
  mc_ mb --ignore-existing "lab5/$BUCKET" >/dev/null
...
POLICY
  mc_ admin policy create lab5 "$POLICY" "$MINIO_DIR/policy.json" >/dev/null 2>&1 || true
  mc_ admin user add lab5 "$access_key" "$secret_key" >/dev/null 2>&1 || true
  mc_ admin policy attach lab5 "$POLICY" --user "$access_key" >/dev/null 2>&1 || true
...
do_status() {
  if running; then
    ...
    mc_ ls "lab5/$BUCKET" 2>/dev/null | sed 's/^/  /' || true
