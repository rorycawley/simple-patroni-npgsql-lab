...
readonly APP_VM=lab5-app1
readonly PKI=/etc/lab5/pki
...
primary_json="$(limactl shell --tty=false lab5-pg1 sudo -u postgres \
  patronictl -c /etc/patroni/patroni.yml list --format=json 2>/dev/null)"
primary="lab5-$(jq -r '.[] | select(.Role | test("Leader")) | .Member' <<< "$primary_json")"
...
out="$(limactl shell --tty=false "$APP_VM" sudo env \
  LAB5_PG_HOSTS="${PG1_IP},${PG2_IP},${PG3_IP}" \
  LAB5_PGPASS=/etc/lab5/client/pgpass LAB5_CA=/etc/lab5/client/ca.crt \
  /opt/lab5-client/Lab4.Client 2>/dev/null | jq -r '.ok' 2>/dev/null || true)"
...
out="$(limactl shell --tty=false "$APP_VM" sudo bash -c \
  "PGPASSFILE=/etc/lab5/client/pgpass psql 'host=$primary_ip port=5432 dbname=appdb user=app_runtime sslmode=disable connect_timeout=5' -Atc 'select 1' >/dev/null 2>&1 && echo ACCEPTED || echo refused" 2>/dev/null || echo refused)"
