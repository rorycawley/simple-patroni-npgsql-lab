...
readonly GATE=/usr/local/lib/lab5/lab5-leader-gate
...
broken_gate() {  # $1=sed expression, $2=description
  local rc
  on "$leader" sudo sed "$1" "$GATE" > /tmp/lab5-gate-broken.sh
  on "$leader" sudo tee "$GATE-broken" >/dev/null < /tmp/lab5-gate-broken.sh
...
for vm in "${VM_NAMES[@]}"; do
  out="$(on "$vm" sudo systemctl start "lab5-backup@full.service" 2>&1)"; rc=$?
...
for vm in "${VM_NAMES[@]}"; do
  [[ "$vm" == "$leader" ]] && continue
  if on "$vm" journalctl -u "lab5-backup@full.service" -n 20 --no-pager \
...
on "$leader" sudo systemctl start "lab5-backup@incr.service" >/dev/null 2>&1
on "$leader" sudo systemctl start "lab5-backup@incr.service" >/dev/null 2>&1
final="$(repo_json)"
...
for vm in "${VM_NAMES[@]}"; do
  for timer in lab5-backup-full.timer lab5-backup-incr.timer; do
