...
// Proves Command Timeout actually bounds a running statement. Lab 1 sets no
...
{
    var probeId = Environment.GetEnvironmentVariable("LAB1_PROBE_ID")
        ?? throw new InvalidOperationException("LAB1_PROBE_ID is required for the uncertain-write probe.");
    var applicationName = Environment.GetEnvironmentVariable("LAB1_APP_NAME")
        ?? throw new InvalidOperationException("LAB1_APP_NAME is required for the uncertain-write probe.");
    ...
    if (probeId.Length is 0 or > 64 || !probeId.All(char.IsAsciiHexDigit))
        throw new InvalidOperationException("LAB1_PROBE_ID must be non-empty hex.");
