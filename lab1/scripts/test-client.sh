...
source "$LAB_DIR/.env"
export LAB1_PG_HOSTS="${PG1_IP},${PG2_IP},${PG3_IP}"
export LAB1_PGPASS="$LAB_DIR/.secrets/pgpass"
...
run_client() {
  dotnet run --project "$LAB_DIR/client/Lab1.Client.csproj" --configuration Release -- "$@"
}
...
test_uncertain_write() {
  ...
  app_name="lab1-uncertain-${probe_id:0:8}"
  ...
  client_stdout="$(mktemp "${TMPDIR:-/tmp}/lab1-uncertain-out.XXXXXX")"
  client_stderr="$(mktemp "${TMPDIR:-/tmp}/lab1-uncertain-err.XXXXXX")"

  LAB1_PROBE_ID="$probe_id" LAB1_APP_NAME="$app_name" \
