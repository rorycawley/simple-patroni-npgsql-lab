...
[Lab 4 guide](../README.md). This file covers how it is applied.
...
| `app-client.yml` | Provisions `lab4-app1` with the client, the CA, and the password file |
...
- installs `lab4-require-volume`, a guard the service units call from
  `ExecStartPre`.
...
`roles/app_client` installs the client on `lab4-app1` as a self-contained
...
`pg1.lab4.example`, `pg2.lab4.example`, `pg3.lab4.example` and `app1.lab4.example`
in macOS `/etc/hosts`.
