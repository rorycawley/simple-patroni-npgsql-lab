...
readonly VM_HOSTNAMES=(pg1.lab4.example pg2.lab4.example pg3.lab4.example)
...
readonly APP_HOSTNAME=app1.lab4.example
...
# Somewhere to restore a copy to, encrypted like everything else. Sized above
# pgdata because a restore holds the same data plus room to work, and distinct
# from every other disk because the guest identifies devices by size.
readonly VM_RESTORE_GIB="${VM_RESTORE_GIB:-12}"
readonly LIMA_TEMPLATE="${LIMA_TEMPLATE:-$SCRIPT_DIR/../rocky-9.8.yaml}"
readonly LIMA_NETWORK="${LIMA_NETWORK:-lima:shared}"
readonly ENV_FILE="$SCRIPT_DIR/../.env"
readonly HOSTS_FILE="/etc/hosts"
readonly HOSTS_BEGIN="# BEGIN simple-patroni-npgsql-lab lab4"
readonly HOSTS_END="# END simple-patroni-npgsql-lab lab4"

usage() {
  cat <<'EOF'
Usage: ./scripts/vms.sh create|destroy|status|hostnames

create   Create (if needed) and start lab4-pg1, lab4-pg2, lab4-pg3, and lab4-app1.
destroy  Permanently delete those four VMs and their disks.
status   Display the Lima status of the Lab 4 VMs.
hostnames  Update the marked Lab 4 block in macOS /etc/hosts.
...
data_disks_for() {
  printf '%s-pgdata %s-etcd %s-restore\n' "$1" "$1" "$1"
}
...
configure_hostnames() {
  ...
  hosts_without_block="$(mktemp "${TMPDIR:-/tmp}/lab4-hosts.XXXXXX")"
  updated_hosts="$(mktemp "${TMPDIR:-/tmp}/lab4-hosts-updated.XXXXXX")"
  ...
  [[ "${LAB4_INTERACTIVE_SUDO:-0}" == 1 ]] && sudo_options=()
...
remove_hostnames() {
  ...
  if ! sudo "${sudo_options[@]}" cp "$hosts_without_block" "$HOSTS_FILE"; then
    echo "Could not remove the Lab 4 host aliases without interactive sudo." >&2
    rm -f "$hosts_without_block"
    return
  fi
  rm -f "$hosts_without_block"
  echo "Removed Lab 4 hostnames from $HOSTS_FILE"
}
...
create() {
  for vm_name in "${VM_NAMES[@]}"; do
    if instance_exists "$vm_name"; then
      ...
      ensure_disk "${vm_name}-restore" "$VM_RESTORE_GIB"
      limactl create --tty=false --name="$vm_name" \
        --cpus="$VM_CPUS" --memory="$VM_MEMORY_GIB" --disk="$VM_DISK_GIB" \
        --mount-none --network="$LIMA_NETWORK" \
        --set ".additionalDisks = [{\"name\":\"${vm_name}-pgdata\"},{\"name\":\"${vm_name}-etcd\"},{\"name\":\"${vm_name}-restore\"}]" \
        "$LIMA_TEMPLATE"
    fi
...
main() {
  ...
  case "${1:-}" in
    ...
    destroy) LAB4_INTERACTIVE_SUDO=1 destroy ;;
    status) status ;;
    hostnames) require_network; LAB4_INTERACTIVE_SUDO=1 configure_hostnames ;;
    *) usage; exit 2 ;;
  esac
}
