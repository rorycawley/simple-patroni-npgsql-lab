...
# The backup object store for Lab 4, on the control machine.
#
...
#   lab4-pgbackrest reads and writes ONE bucket, and nothing else
#
...
# Not 9000 (the sibling lab ../percona-patroni-npgsql-lab holds it) and not 9100
# (Lab 3 holds that). Every lab that runs an object store needs its own port, or
# it cannot be built while another one is running -- the same standalone property
# that keeps Labs 1 and 2 independent copies.
readonly PORT="${LAB4_MINIO_PORT:-9200}"
readonly POLICY=lab4-backup-rw
...
do_start() {
  ...
  cp "$PKI_DIR/ca.crt" "$MC_DIR/certs/CAs/lab4-ca.crt"
  ...
  mc_ alias set lab4 "$(endpoint)" "$root_user" "$root_pass" >/dev/null
  mc_ mb --ignore-existing "lab4/$BUCKET" >/dev/null
...
POLICY
  mc_ admin policy create lab4 "$POLICY" "$MINIO_DIR/policy.json" >/dev/null 2>&1 || true
  mc_ admin user add lab4 "$access_key" "$secret_key" >/dev/null 2>&1 || true
  mc_ admin policy attach lab4 "$POLICY" --user "$access_key" >/dev/null 2>&1 || true
...
do_status() {
  if running; then
    ...
    mc_ ls "lab4/$BUCKET" 2>/dev/null | sed 's/^/  /' || true
