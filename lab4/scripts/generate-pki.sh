...
#   minio               serverAuth              The backup object store (Lab 4)
#
# minio is not per-node: one object store on the control machine serves every
# node, so it is issued once against the Lima gateway address the guests reach
# it on. It extends the existing CA rather than introducing a second trust root,
# which is the decision recorded in lab2/PLAN.md before this lab began.
#
...
if [[ ! -f "$PKI_DIR/ca.crt" ]]; then
  ...
  openssl req -x509 -new -sha256 -days "$CA_DAYS" -key "$PKI_DIR/ca.key" \
    -out "$PKI_DIR/ca.crt" -subj "/CN=Lab 4 Root CA/O=Local Lab"
fi
...
# network gateway. host.lima.internal is how a guest names the host; the IP is
# what pgBackRest actually connects to, and under verification only an
# IP Address SAN can satisfy a connection made by address.
gateway_ip="${LAB4_GATEWAY_IP:-${PG1_IP%.*}.1}"
[[ -n "$gateway_ip" ]] || { echo "Cannot derive the gateway address" >&2; exit 1; }
sign_cert "minio" "minio.${DOMAIN}" \
  "DNS:minio.${DOMAIN},DNS:host.lima.internal,DNS:localhost,IP:${gateway_ip},IP:127.0.0.1" \
  "serverAuth"
