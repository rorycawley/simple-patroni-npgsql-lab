...
strays="$(cat $strays 2>/dev/null | sed 's/#.*$//' \
  | grep -ohE 'lab[0-9]' | sort -u | grep -v '^lab4$' | tr '\n' ' ')"
if [[ -z "$strays" ]]; then
...
if mc --quiet --no-color alias set lab4backup "$ENDPOINT" \
...
mc --quiet --no-color ls "lab4backup/$BUCKET" >/dev/null 2>&1 \
...
if mc --quiet --no-color mb "lab4backup/should-not-exist" >/dev/null 2>&1; then
  fail "the backup credential created a second bucket; the policy is not scoped"
  mc --quiet --no-color rb --force "lab4backup/should-not-exist" >/dev/null 2>&1 || true
else
