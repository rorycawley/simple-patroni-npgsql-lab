...
readonly TIMERS=(lab4-backup-full.timer lab4-backup-incr.timer lab4-dump.timer)
...
for vm in "${VM_NAMES[@]}"; do
  on "$vm" sudo systemctl start "lab4-backup@incr.service" >/dev/null 2>&1
done
if on "$after_leader" journalctl -u "lab4-backup@incr.service" -n 20 --no-pager | grep -q "Leader confirmed"; then
