...
run_client() {
  ...
  limactl shell --tty=false lab4-app1 sudo env "${env_pairs[@]}" \
...
test_uncertain_write() {
  ...
  app_name="lab4-uncertain-${probe_id:0:8}"
  ...
  client_stdout="$(mktemp "${TMPDIR:-/tmp}/lab4-uncertain-out.XXXXXX")"
  client_stderr="$(mktemp "${TMPDIR:-/tmp}/lab4-uncertain-err.XXXXXX")"

  run_client "LAB4_PROBE_ID=$probe_id" "LAB4_APP_NAME=$app_name" uncertain-write > "$client_stdout" 2> "$client_stderr" &
  client_pid=$!
