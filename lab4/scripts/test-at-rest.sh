...
cleanup() {
  if [[ -n "$broken_vm" ]]; then
    ...
    limactl shell --tty=false "$broken_vm" sudo bash -c '
      ...
      cryptsetup status lab4-pgdata >/dev/null 2>&1 || \
        cryptsetup luksOpen --key-file /etc/lab4/luks/pgdata.key "$device" lab4-pgdata
      [ -f /etc/fstab.lab4bak ] && mv /etc/fstab.lab4bak /etc/fstab
...
for spec in "/var/lib/pgsql:lab4-pgdata" "/var/lib/etcd:lab4-etcd"; do
...
test_guard() {
  ...
  target=""
  for vm in "${VM_NAMES[@]}"; do
    if [[ "$vm" != "lab4-$leader" ]]; then target="$vm"; break; fi
  done
...
set -uo pipefail
fail=0
device=$(cryptsetup status lab4-pgdata | awk '/device:/ {print $2}')
...
check_case() {
  ...
  if [ "$state" = "active" ]; then
    if [ "$src" = "/dev/mapper/lab4-pgdata" ]; then
...
cryptsetup luksClose lab4-pgdata
...
cp /etc/fstab /etc/fstab.lab4bak
sed -i '/lab4-pgdata/d' /etc/fstab
systemctl daemon-reload
