...
require_vms() {
  command -v limactl >/dev/null 2>&1 || {
    echo "limactl is required to build the Lab 2 inventory" >&2
    exit 1
  }
  [[ -f "$ENV_FILE" ]] || {
...
inventory() {
  ...
  for index in "${!VM_NAMES[@]}"; do
    ...
    [[ "$index" -gt 0 ]] && printf ','
    printf '"%s":{"ansible_host":"lima-%s","ansible_ssh_common_args":"-F %s","lab2_node_ip":"%s","lab2_node_name":"%s","lab2_node_fqdn":"%s"}' \
