...
readonly STRICT_PROBE_APP=lab2-sync-strict-probe
...
strict_fault_probe() {
  ...
  block_output="$(mktemp "${TMPDIR:-/tmp}/lab2-strict.XXXXXX")"
  limactl shell --tty=false "$primary" sudo -u postgres \
...
test_durability() {
  ...
  # "lab2-" is what leader_vm returns when patronictl reports no leader at all:
  ...
  [[ -n "$new_primary" && "$new_primary" != "$primary" && "$new_primary" != "lab2-" ]] \
