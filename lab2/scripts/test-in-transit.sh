...
primary_json="$(limactl shell --tty=false lab2-pg1 sudo -u postgres \
  patronictl -c /etc/patroni/patroni.yml list --format=json 2>/dev/null)"
primary="lab2-$(jq -r '.[] | select(.Role | test("Leader")) | .Member' <<< "$primary_json")"
...
out="$(limactl shell --tty=false "$APP_VM" sudo env \
  ...
  LAB2_PGPASS=/etc/lab2/client/pgpass LAB2_CA=/etc/lab2/client/ca.crt \
  /opt/lab2-client/Lab2.Client 2>/dev/null | jq -r '.ok' 2>/dev/null || true)"
...
out="$(limactl shell --tty=false "$APP_VM" sudo bash -c \
  "PGPASSFILE=/etc/lab2/client/pgpass psql 'host=$primary_ip port=5432 dbname=appdb user=app_runtime sslmode=disable connect_timeout=5' -Atc 'select 1' >/dev/null 2>&1 && echo ACCEPTED || echo refused" 2>/dev/null || echo refused)"
