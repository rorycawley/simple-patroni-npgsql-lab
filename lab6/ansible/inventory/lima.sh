...
readonly NODE_FQDNS=(pg1.lab6.example pg2.lab6.example pg3.lab6.example)
readonly APP_VM=lab6-app1
readonly APP_FQDN=app1.lab6.example
...
require_vms() {
  command -v limactl >/dev/null 2>&1 || {
    echo "limactl is required to build the Lab 6 inventory" >&2
    exit 1
  }
  [[ -f "$ENV_FILE" ]] || {
...
inventory() {
  ...
  for index in "${!VM_NAMES[@]}"; do
    ...
    [[ "$index" -gt 0 ]] && printf ','
    printf '"%s":{"ansible_host":"lima-%s","ansible_ssh_common_args":"-F %s","lab6_node_ip":"%s","lab6_node_name":"%s","lab6_node_fqdn":"%s"}' \
  ...
  printf ',"%s":{"ansible_host":"lima-%s","ansible_ssh_common_args":"-F %s","lab6_node_ip":"%s","lab6_node_name":"app1","lab6_node_fqdn":"%s"}' \
