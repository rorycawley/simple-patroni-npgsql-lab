...
while IFS= read -r label; do
  [[ -z "$label" ]] && continue
  found="$(on "$node" sudo "$BIN/lab6-s3" list "pgbackrest/backup/$STANZA/$label/" | grep -c . || true)"
...
# not just its rows -- and Lab 6's recovery depends on it, so it is asserted here
