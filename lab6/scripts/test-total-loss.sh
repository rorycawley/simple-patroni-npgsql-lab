  ...
  for vm in "${VM_NAMES[@]}"; do
    ...
    on "$vm" sudo bash -c \
      'systemctl is-active etcd; journalctl -u etcd --no-pager -n 6 2>&1 | tail -6;
       echo "member dir: $(ls -A /var/lib/etcd/lab6 2>/dev/null | wc -l) entries"' 2>&1 \
      | sed 's/^/    /' >&2
  done
  exit 1
fi
...
for vm in "${VM_NAMES[@]}"; do
  on "$vm" sudo systemctl stop lab6-backup-full.timer lab6-backup-incr.timer lab6-dump.timer \
    >/dev/null 2>&1
done
...
for vm in "${VM_NAMES[@]}"; do
  on "$vm" sudo systemctl start lab6-backup-full.timer lab6-backup-incr.timer lab6-dump.timer \
    >/dev/null 2>&1
done
running_timers=0
for vm in "${VM_NAMES[@]}"; do
  [[ "$(on "$vm" systemctl is-active lab6-backup-incr.timer)" == "active" ]] \
