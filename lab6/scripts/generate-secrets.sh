...
command -v openssl >/dev/null 2>&1 || {
  echo "openssl is required to generate Lab 6 passwords" >&2
  exit 1
}
...
ensure_secret "$REPO_SECRETS" minio_root_user         "lab6-admin"
...
ensure_secret "$REPO_SECRETS" telemetry_access_key  "lab6-telemetry"
ensure_secret "$REPO_SECRETS" telemetry_secret_key  "$(openssl rand -hex 24)"
ensure_secret "$REPO_SECRETS" minio_backup_access_key "lab6-pgbackrest"
...
# LAB6_REPO_CIPHER_PASS.
if ! grep -q '^repo_cipher_pass:' "$REPO_SECRETS"; then
  printf 'repo_cipher_pass: "%s"\n' \
    "${LAB6_REPO_CIPHER_PASS:-$(openssl rand -hex 32)}" >> "$REPO_SECRETS"
...
if ! grep -q '^dump_cipher_pass:' "$REPO_SECRETS"; then
  printf 'dump_cipher_pass: "%s"\n' \
    "${LAB6_DUMP_CIPHER_PASS:-$(openssl rand -hex 32)}" >> "$REPO_SECRETS"
