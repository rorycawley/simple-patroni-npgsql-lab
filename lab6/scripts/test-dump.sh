...
cleanup() {
  ...
  on "$leader" sudo rm -f /tmp/lab6-dump-probe.enc /tmp/lab6-dump-probe >/dev/null 2>&1
}
trap cleanup EXIT
...
before="$(on "$leader" sudo "$BIN/lab6-s3" list dumps/ | wc -l | tr -d ' ')"

for vm in "${VM_NAMES[@]}"; do
  out="$(on "$vm" sudo systemctl start lab6-dump.service 2>&1)"; rc=$?
  (( rc == 0 )) || fail "${vm#$VM_PREFIX}: the dump unit failed: $out"
done

for vm in "${VM_NAMES[@]}"; do
  [[ "$vm" == "$leader" ]] && continue
  on "$vm" journalctl -u lab6-dump.service -n 10 --no-pager | grep -q "Leader confirmed" \
    && fail "${vm#$VM_PREFIX}: a standby took a dump" \
    || pass "${vm#$VM_PREFIX}: declined, as a non-leader must"
done

keys="$(on "$leader" sudo "$BIN/lab6-s3" list dumps/ | sort)"
newest="$(printf '%s\n' "$keys" | tail -1)"
...
on "$leader" sudo bash -c "$BIN/lab6-s3 cat '$newest' > /tmp/lab6-dump-probe.enc"
raw_head="$(on "$leader" sudo bash -c \
  "head -c 8 /tmp/lab6-dump-probe.enc | od -An -v -tx1 | tr -d ' \n'")"
...
on "$leader" sudo bash -c "
  openssl enc -d -aes-256-cbc -pbkdf2 -in /tmp/lab6-dump-probe.enc \
    -out /tmp/lab6-dump-probe -pass file:$PASSFILE" >/dev/null 2>&1 \
  && pass "decrypts with the dump passphrase" \
  || { fail "could not decrypt the dump"; exit 1; }

on "$leader" sudo -u postgres "$PGBIN/pg_restore" --list /tmp/lab6-dump-probe >/dev/null 2>&1 \
...
on "$leader" sudo -u postgres "$PGBIN/pg_restore" -d "$SCRATCH" /tmp/lab6-dump-probe >/dev/null 2>&1
...
sql_text="$(on "$leader" sudo -u postgres "$PGBIN/pg_restore" -f - /tmp/lab6-dump-probe 2>/dev/null)"
...
count="$(printf '%s\n' "$keys" | grep -c . || true)"
keep="$(on "$leader" sudo sed -n 's/^readonly KEEP=//p' "$BIN/lab6-dump")"
(( count <= keep )) \
