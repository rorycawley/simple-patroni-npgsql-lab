...
# Fault-injection tests for the Lab 6 cluster.
#
...
readonly FENCE_TABLE=lab6_fence
...
diagnose_election() {
  ...
  limactl shell --tty=false "$survivor" sudo bash -c \
    "etcdctl --cacert=/etc/lab6/pki/ca.crt --cert=/etc/lab6/pki/etcd.crt \
     --key=/etc/lab6/pki/etcd.key \
...
run_scenario() {
  ...
  client_stdout="$(mktemp "${TMPDIR:-/tmp}/lab6-failover-out.XXXXXX")"
  client_stderr="$(mktemp "${TMPDIR:-/tmp}/lab6-failover-err.XXXXXX")"
