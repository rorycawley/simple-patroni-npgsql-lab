...
cleanup() {
  if [[ -n "$broken_vm" ]]; then
    ...
    limactl shell --tty=false "$broken_vm" sudo bash -c '
      ...
      cryptsetup status lab6-pgdata >/dev/null 2>&1 || \
        cryptsetup luksOpen --key-file /etc/lab6/luks/pgdata.key "$device" lab6-pgdata
      [ -f /etc/fstab.lab6bak ] && mv /etc/fstab.lab6bak /etc/fstab
...
for spec in "/var/lib/pgsql:lab6-pgdata" "/var/lib/etcd:lab6-etcd"; do
...
test_guard() {
  ...
  target=""
  for vm in "${VM_NAMES[@]}"; do
    if [[ "$vm" != "lab6-$leader" ]]; then target="$vm"; break; fi
  done
...
set -uo pipefail
fail=0
device=$(cryptsetup status lab6-pgdata | awk '/device:/ {print $2}')
...
check_case() {
  ...
  if [ "$state" = "active" ]; then
    if [ "$src" = "/dev/mapper/lab6-pgdata" ]; then
...
cryptsetup luksClose lab6-pgdata
...
cp /etc/fstab /etc/fstab.lab6bak
sed -i '/lab6-pgdata/d' /etc/fstab
systemctl daemon-reload
