...
# The backup object store for Lab 6, on the control machine.
#
...
#   lab6-pgbackrest reads and writes ONE bucket, and nothing else
#
...
readonly PORT="${LAB6_MINIO_PORT:-9400}"
readonly POLICY=lab6-backup-rw
...
do_start() {
  ...
  cp "$PKI_DIR/ca.crt" "$MC_DIR/certs/CAs/lab6-ca.crt"
  ...
  mc_ alias set lab6 "$(endpoint)" "$root_user" "$root_pass" >/dev/null
  mc_ mb --ignore-existing "lab6/$BUCKET" >/dev/null
...
POLICY
  mc_ admin policy create lab6 "$POLICY" "$MINIO_DIR/policy.json" >/dev/null 2>&1 || true
  mc_ admin user add lab6 "$access_key" "$secret_key" >/dev/null 2>&1 || true
  mc_ admin policy attach lab6 "$POLICY" --user "$access_key" >/dev/null 2>&1 || true
...
do_status() {
  if running; then
    ...
    mc_ ls "lab6/$BUCKET" 2>/dev/null | sed 's/^/  /' || true
