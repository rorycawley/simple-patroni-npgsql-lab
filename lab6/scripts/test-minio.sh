...
readonly GUEST_CA=/etc/lab6/pki/ca.crt
...
strays="$(cat $strays 2>/dev/null | sed 's/#.*$//' \
  | grep -ohE 'lab[0-9]' | sort -u | grep -v '^lab6$' | tr '\n' ' ')"
if [[ -z "$strays" ]]; then
...
if mc --quiet --no-color alias set lab6backup "$ENDPOINT" \
...
mc --quiet --no-color ls "lab6backup/$BUCKET" >/dev/null 2>&1 \
...
if mc --quiet --no-color mb "lab6backup/should-not-exist" >/dev/null 2>&1; then
  fail "the backup credential created a second bucket; the policy is not scoped"
  mc --quiet --no-color rb --force "lab6backup/should-not-exist" >/dev/null 2>&1 || true
else
