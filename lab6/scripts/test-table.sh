...
on "$leader" sudo systemctl start lab6-dump.service >/dev/null 2>&1
newest="$(on "$leader" sudo "$BIN/lab6-s3" list dumps/ | sort | tail -1)"
[[ -n "$newest" ]] \
...
on "$leader" sudo bash -c "
  $BIN/lab6-s3 cat '$newest' > /tmp/rung4.enc &&
