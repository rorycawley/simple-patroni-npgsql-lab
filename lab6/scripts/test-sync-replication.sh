...
readonly BLOCK_PROBE_APP=lab6-sync-block-probe
...
readonly STRICT_PROBE_APP=lab6-sync-strict-probe
...
test_blocking() {
  ...
  block_output="$(mktemp "${TMPDIR:-/tmp}/lab6-syncblock.XXXXXX")"
...
strict_fault_probe() {
  ...
  block_output="$(mktemp "${TMPDIR:-/tmp}/lab6-strict.XXXXXX")"
  limactl shell --tty=false "$primary" sudo -u postgres \
...
test_durability() {
  ...
  for _ in {1..45}; do
    ...
    [[ -n "$new_primary" && "$new_primary" != "$primary" && "$new_primary" != "lab6-" ]] && break
    sleep 2
  done
  # "lab6-" is what leader_vm returns when patronictl reports no leader at all:
  ...
  [[ -n "$new_primary" && "$new_primary" != "$primary" && "$new_primary" != "lab6-" ]] \
