...
#   minio               serverAuth              The backup object store (Lab 6)
#
...
if [[ ! -f "$PKI_DIR/ca.crt" ]]; then
  ...
  openssl req -x509 -new -sha256 -days "$CA_DAYS" -key "$PKI_DIR/ca.key" \
    -out "$PKI_DIR/ca.crt" -subj "/CN=Lab 6 Root CA/O=Local Lab"
fi
...
gateway_ip="${LAB6_GATEWAY_IP:-${PG1_IP%.*}.1}"
