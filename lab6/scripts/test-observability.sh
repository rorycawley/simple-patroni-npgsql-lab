...
readonly BACKUP_BUCKET=lab6-backups
...
readonly LOKI="http://$GW:${LAB6_LOKI_PORT:-3110}"
...
for _ in $(seq 1 40); do
  mimir_blocks="$(mc_ ls --recursive "lab6/$MIMIR_BUCKET" 2>/dev/null | grep -vc "seed" || true)"
  loki_chunks="$(mc_ ls --recursive "lab6/$LOKI_BUCKET" 2>/dev/null | grep -vc "seed" || true)"
...
checked="$(promq "max(lab6_repo_wal_checked)")"
age="$(promq "max(lab6_repo_last_backup_age_seconds)")"
(( ${age%%.*} >= 0 )) \
  && pass "the newest backup is ${age%%.*}s old, reported by the node holding the leader key" \
  || fail "no backup age is being reported"
leaders="$(promq "count(lab6_repo_metrics_leader == 1)")"
[[ "${leaders%%.*}" == "1" ]] \
...
MAILPIT="http://$GW:${LAB6_MAILPIT_PORT:-8035}"
...
mc_ alias set lab6tel "https://$GW:${LAB6_MINIO_PORT:-9400}" "$tel_key" "$tel_secret" >/dev/null 2>&1
mc_ ls "lab6tel/$MIMIR_BUCKET" >/dev/null 2>&1 \
  && pass "the telemetry credential can read its own bucket" \
  || fail "the telemetry credential cannot reach $MIMIR_BUCKET"
if mc_ ls "lab6tel/$BACKUP_BUCKET" >/dev/null 2>&1; then
...
mc_ alias remove lab6tel >/dev/null 2>&1
