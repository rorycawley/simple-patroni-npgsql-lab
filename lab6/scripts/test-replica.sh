...
# on `streaming` alone recorded a 1s rebuild -- pg2 was still publishing the
# state it held BEFORE the reinit took effect -- and the journal was then read
# before pgBackRest had written a line of its report, so the method assertions
# below failed against a rebuild that had barely begun.
#
