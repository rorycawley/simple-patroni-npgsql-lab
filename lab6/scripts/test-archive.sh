...
readonly TIMERS=(lab6-backup-full.timer lab6-backup-incr.timer lab6-dump.timer)
...
for vm in "${VM_NAMES[@]}"; do
  on "$vm" sudo systemctl start "lab6-backup@incr.service" >/dev/null 2>&1
done
if on "$after_leader" journalctl -u "lab6-backup@incr.service" -n 20 --no-pager | grep -q "Leader confirmed"; then
