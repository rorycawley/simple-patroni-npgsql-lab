...
run_client() {
  ...
  limactl shell --tty=false lab6-app1 sudo env "${env_pairs[@]}" \
...
test_uncertain_write() {
  ...
  app_name="lab6-uncertain-${probe_id:0:8}"
  ...
  client_stdout="$(mktemp "${TMPDIR:-/tmp}/lab6-uncertain-out.XXXXXX")"
  client_stderr="$(mktemp "${TMPDIR:-/tmp}/lab6-uncertain-err.XXXXXX")"

  run_client "LAB6_PROBE_ID=$probe_id" "LAB6_APP_NAME=$app_name" uncertain-write > "$client_stdout" 2> "$client_stderr" &
  client_pid=$!
