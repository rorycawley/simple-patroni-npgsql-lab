...
#
# STORAGE IS MINIO. Mimir keeps its blocks in `lab6-mimir` and Loki its chunks in
# `lab6-loki`, which is how both are run anywhere that matters, and which this lab
...
readonly MIMIR_BUCKET=lab6-mimir
readonly LOKI_BUCKET=lab6-loki
readonly TELEMETRY_POLICY=lab6-telemetry-rw
...
readonly GRAFANA_PORT="${LAB6_GRAFANA_PORT:-3010}"
readonly MIMIR_PORT="${LAB6_MIMIR_PORT:-9019}"
readonly LOKI_PORT="${LAB6_LOKI_PORT:-3110}"
readonly MAILPIT_HTTP_PORT="${LAB6_MAILPIT_PORT:-8035}"
readonly ALERTMANAGER_PORT="${LAB6_ALERTMANAGER_PORT:-9103}"
...
readonly NET=lab6-observability
...
gateway_ip() {
  ...
  source "$LAB_DIR/.env"
  printf '%s\n' "${LAB6_GATEWAY_IP:-${PG1_IP%.*}.1}"
}

docker_() { docker "$@"; }
running() { [[ -n "$(docker_ ps -q -f "name=^lab6-(grafana|mimir|loki|mailpit)$" 2>/dev/null)" ]]; }
...
readonly S3_FROM_CONTAINER="host.lima.internal:${LAB6_MINIO_PORT:-9400}"
...
ensure_buckets() {
  ...
  mc_ alias set lab6 "https://$(gateway_ip):${LAB6_MINIO_PORT:-9400}" \
    "$root_user" "$root_pass" >/dev/null 2>&1

  local b
  for b in "$MIMIR_BUCKET" "$LOKI_BUCKET"; do
    mc_ mb --ignore-existing "lab6/$b" >/dev/null 2>&1
  done
...
POLICY
  mc_ admin policy create lab6 "$TELEMETRY_POLICY" "$OBS_DIR/telemetry-policy.json" >/dev/null 2>&1
  mc_ admin user add lab6 "$tel_key" "$tel_secret" >/dev/null 2>&1
  mc_ admin policy attach lab6 "$TELEMETRY_POLICY" --user "$tel_key" >/dev/null 2>&1
  return 0
}
...
ruler:
  ...
  # does not.
  alertmanager_url: http://lab6-alertmanager:9093/
ruler_storage:
  s3:
    bucket_name: ${MIMIR_BUCKET}
...
LOKI
  ...
  cat > "$CONF_DIR/provisioning/datasources/lab6.yaml" <<DS
apiVersion: 1
datasources:
  - name: Mimir
    type: prometheus
    access: proxy
    url: http://lab6-mimir:${MIMIR_PORT}/prometheus
    isDefault: true
  - name: Loki
    type: loki
    access: proxy
    url: http://lab6-loki:${LOKI_PORT}
DS
}
...
do_start() {
  ...
  docker_ run -d --name lab6-loki --network "$NET" \
  ...
  docker_ run -d --name lab6-mailpit --network "$NET" \
...
receivers:
  - name: lab6-oncall
    email_configs:
      - to: oncall@lab6.example
        from: alertmanager@lab6.example
        smarthost: lab6-mailpit:1025
        require_tls: false
        send_resolved: true
AMCFG

  docker_ run -d --name lab6-alertmanager --network "$NET" \
  ...
  docker_ run -d --name lab6-grafana --network "$NET" \
    -p "${GRAFANA_PORT}:3000" \
  ...
  if [[ -n "$ready" ]]; then
    ...
    echo "The stack did not become ready; see 'docker logs lab6-mimir' / 'lab6-loki'" >&2
    return 1
  fi
}

do_stop() {
  docker_ rm -f lab6-grafana lab6-loki lab6-mimir lab6-mailpit lab6-alertmanager >/dev/null 2>&1
...
do_status() {
  if running; then
    echo "Running: $(url_)"
    docker_ ps --filter "name=lab6-" --format '  {{.Names}}  {{.Status}}'
  else
    echo "Not running"
  fi
}
...
do_destroy() {
  ...
  MC_CONFIG_DIR="$mc_dir" mc --quiet --no-color alias set lab6 \
    "https://$(gateway_ip):${LAB6_MINIO_PORT:-9400}" "$root_user" "$root_pass" >/dev/null 2>&1
  local b
  for b in "$MIMIR_BUCKET" "$LOKI_BUCKET"; do
    MC_CONFIG_DIR="$mc_dir" mc --quiet --no-color rb --force "lab6/$b" >/dev/null 2>&1
  done
