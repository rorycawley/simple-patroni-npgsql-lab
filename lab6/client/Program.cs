...
var hosts = Environment.GetEnvironmentVariable("LAB6_PG_HOSTS")
    ?? throw new InvalidOperationException("LAB6_PG_HOSTS is required.");
var passfile = Environment.GetEnvironmentVariable("LAB6_PGPASS")
    ?? throw new InvalidOperationException("LAB6_PGPASS is required.");
var rootCertificate = Environment.GetEnvironmentVariable("LAB6_CA")
    ?? throw new InvalidOperationException("LAB6_CA is required.");
...
// Proves Command Timeout actually bounds a running statement. Lab 6 sets no
...
{
    var probeId = Environment.GetEnvironmentVariable("LAB6_PROBE_ID")
        ?? throw new InvalidOperationException("LAB6_PROBE_ID is required for the uncertain-write probe.");
    var applicationName = Environment.GetEnvironmentVariable("LAB6_APP_NAME")
        ?? throw new InvalidOperationException("LAB6_APP_NAME is required for the uncertain-write probe.");
    ...
    if (probeId.Length is 0 or > 64 || !probeId.All(char.IsAsciiHexDigit))
        throw new InvalidOperationException("LAB6_PROBE_ID must be non-empty hex.");
