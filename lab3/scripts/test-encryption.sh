...
head_hex="$(on "$node" sudo bash -c \
  "$BIN/lab3-s3 cat pgbackrest/backup/$STANZA/backup.info 2>/dev/null | head -c 8 | od -An -v -tx1 | tr -d ' \n'")"
...
newest_dump="$(on "$node" sudo "$BIN/lab3-s3" list dumps/ | sort | tail -1)"
if [[ -z "$newest_dump" ]]; then
  fail "no dump found to check"
else
  head_hex="$(on "$node" sudo bash -c \
    "$BIN/lab3-s3 cat '$newest_dump' 2>/dev/null | head -c 8 | od -An -v -tx1 | tr -d ' \n'")"
  if [[ "$head_hex" == 53616c7465645f5f ]]; then
  ...
  if on "$node" sudo bash -c \
      "$BIN/lab3-s3 cat '$newest_dump' > /tmp/lab3-enc-probe 2>/dev/null &&
       openssl enc -d -aes-256-cbc -pbkdf2 -in /tmp/lab3-enc-probe -out /dev/null \
  ...
  on "$node" sudo rm -f /tmp/lab3-enc-probe >/dev/null 2>&1
fi
