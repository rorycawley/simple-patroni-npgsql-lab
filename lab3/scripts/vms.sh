...
readonly VM_HOSTNAMES=(pg1.lab3.example pg2.lab3.example pg3.lab3.example)
...
readonly APP_HOSTNAME=app1.lab3.example
...
readonly HOSTS_BEGIN="# BEGIN simple-patroni-npgsql-lab lab3"
readonly HOSTS_END="# END simple-patroni-npgsql-lab lab3"

usage() {
  cat <<'EOF'
Usage: ./scripts/vms.sh create|destroy|status|hostnames

create   Create (if needed) and start lab3-pg1, lab3-pg2, lab3-pg3, and lab3-app1.
destroy  Permanently delete those four VMs and their disks.
status   Display the Lima status of the Lab 3 VMs.
hostnames  Update the marked Lab 3 block in macOS /etc/hosts.
...
configure_hostnames() {
  ...
  hosts_without_block="$(mktemp "${TMPDIR:-/tmp}/lab3-hosts.XXXXXX")"
  updated_hosts="$(mktemp "${TMPDIR:-/tmp}/lab3-hosts-updated.XXXXXX")"
  ...
  [[ "${LAB3_INTERACTIVE_SUDO:-0}" == 1 ]] && sudo_options=()
...
remove_hostnames() {
  ...
  if ! sudo "${sudo_options[@]}" cp "$hosts_without_block" "$HOSTS_FILE"; then
    echo "Could not remove the Lab 3 host aliases without interactive sudo." >&2
    rm -f "$hosts_without_block"
    return
  fi
  rm -f "$hosts_without_block"
  echo "Removed Lab 3 hostnames from $HOSTS_FILE"
}
...
main() {
  ...
  case "${1:-}" in
    ...
    destroy) LAB3_INTERACTIVE_SUDO=1 destroy ;;
    status) status ;;
    hostnames) require_network; LAB3_INTERACTIVE_SUDO=1 configure_hostnames ;;
    *) usage; exit 2 ;;
  esac
}
