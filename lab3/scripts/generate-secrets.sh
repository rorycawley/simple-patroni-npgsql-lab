...
command -v openssl >/dev/null 2>&1 || {
  echo "openssl is required to generate Lab 3 passwords" >&2
  exit 1
}
...
ensure_secret minio_root_user        "lab3-admin"
ensure_secret minio_root_password    "$(openssl rand -hex 24)"
ensure_secret minio_backup_access_key "lab3-pgbackrest"
...
if ! grep -q '^repo_cipher_pass:' "$REPO_SECRETS"; then
  printf 'repo_cipher_pass: "%s"\n' \
    "${LAB3_REPO_CIPHER_PASS:-$(openssl rand -hex 32)}" >> "$REPO_SECRETS"
...
if ! grep -q '^dump_cipher_pass:' "$REPO_SECRETS"; then
  printf 'dump_cipher_pass: "%s"\n' \
    "${LAB3_DUMP_CIPHER_PASS:-$(openssl rand -hex 32)}" >> "$REPO_SECRETS"
