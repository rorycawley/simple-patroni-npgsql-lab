...
cleanup() {
  if [[ -n "$broken_vm" ]]; then
    ...
    limactl shell --tty=false "$broken_vm" sudo bash -c '
      ...
      cryptsetup status lab3-pgdata >/dev/null 2>&1 || \
        cryptsetup luksOpen --key-file /etc/lab3/luks/pgdata.key "$device" lab3-pgdata
      [ -f /etc/fstab.lab3bak ] && mv /etc/fstab.lab3bak /etc/fstab
...
for spec in "/var/lib/pgsql:lab3-pgdata" "/var/lib/etcd:lab3-etcd"; do
...
test_guard() {
  ...
  target=""
  for vm in "${VM_NAMES[@]}"; do
    if [[ "$vm" != "lab3-$leader" ]]; then target="$vm"; break; fi
  done
...
set -uo pipefail
fail=0
device=$(cryptsetup status lab3-pgdata | awk '/device:/ {print $2}')
...
check_case() {
  ...
  if [ "$state" = "active" ]; then
    if [ "$src" = "/dev/mapper/lab3-pgdata" ]; then
...
cryptsetup luksClose lab3-pgdata
...
cp /etc/fstab /etc/fstab.lab3bak
sed -i '/lab3-pgdata/d' /etc/fstab
systemctl daemon-reload
